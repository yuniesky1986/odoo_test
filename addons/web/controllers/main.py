...
class CSVExport(ExportFormat, http.Controller):
    ...
    def check_date(self, value):
        try:
            parse_data = datetime.datetime.strptime(value, '%Y-%m-%d %H:%M:%S')
            return parse_data
        except Exception as e:
            return False
        ...
        for data in rows:
            ...
            for d in data:
                ...
                if type(d) is str:
                    ...
                    if parse_data:
                        if len(d) > 10:
                            tz = pytz.timezone(request._context.get('tz') or 'UTC')
                            d = (pytz.utc.localize(parse_data).astimezone(tz)).strftime('%Y-%m-%d %H:%M:%S')
